...
| `ipBlock` _string_ | IPBlock specifies the Policy path for IP Block for the allocation.<br />If set, IP addresses of this allocation will be allocated from this IP Block, instead of the default IP Block associated with the Namespace's primary VPC. |  |  |
