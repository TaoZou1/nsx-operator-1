...
func (r *PodReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !podIsDeleted(pod) {
		...
		if subnetPort != nil {
			if r.SubnetPortService.NSXClient.NSXCheckVersion(nsx.StatefulSetPod) && r.isStatefulSetSubnetPort(subnetPort) {
...
func (r *PodReconciler) deleteSubnetPortByPodName(ctx context.Context, ns string, name string) error {
	...
	for _, nsxSubnetPort := range nsxSubnetPorts {
		...
		if r.SubnetPortService.NSXClient.NSXCheckVersion(nsx.StatefulSetPod) && r.isStatefulSetSubnetPort(nsxSubnetPort) {
			log.Info("Ignoring subnet port deletion for StatefulSet pod",
