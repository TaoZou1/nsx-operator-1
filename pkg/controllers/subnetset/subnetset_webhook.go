...
func (v *SubnetSetValidator) validateSubnets(ctx context.Context, ns string, subnetNames *[]string, subnetSet string) (bool, error) {
	...
	firstAccessMode := ""
	firstDHCPMode := ""
	var firstStaticIPAllocation *bool
	isFirstSubnet := true
	if subnetNames == nil {
		return true, nil
	}
	for _, subnetName := range *subnetNames {
		...
		if isFirstSubnet {
			...
			firstStaticIPAllocation = crdSubnet.Spec.AdvancedConfig.StaticIPAllocation.Enabled
			isFirstSubnet = false
		} else {
			...
			staticIPAlloc1 := "nil"
			if firstStaticIPAllocation != nil {
				staticIPAlloc1 = fmt.Sprintf("%t", *firstStaticIPAllocation)
			}
			staticIPAlloc2 := "nil"
			if crdSubnet.Spec.AdvancedConfig.StaticIPAllocation.Enabled != nil {
				staticIPAlloc2 = fmt.Sprintf("%t", *crdSubnet.Spec.AdvancedConfig.StaticIPAllocation.Enabled)
			}
			if staticIPAlloc1 != staticIPAlloc2 {
				return true, fmt.Errorf("Subnets in SubnetSet %s/%s must have the same StaticIPAllocation, found different StaticIPAllocations: [%s, %s]", ns, subnetSet, staticIPAlloc1, staticIPAlloc2)
			}
		}
