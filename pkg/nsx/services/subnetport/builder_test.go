...
func TestBuildSubnetPortIdAndName(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Enable STS feature for "reuse STS port" test, disable for others
			enableStsFeature := tt.name == "reuse STS port by STS UID and pod name"
			patchesNSXCheckVersion := gomonkey.ApplyMethod(reflect.TypeOf(nsxClient), "NSXCheckVersion",
				func(_ *nsx.Client, feature int) bool {
					if feature == nsx.StatefulSetPod {
						return enableStsFeature
					}
					return false
				})
			defer patchesNSXCheckVersion.Reset()

			// Also need to mock GetByKey for normal ID generation path
			patchesGetByKey := gomonkey.ApplyFunc(
				(*SubnetPortStore).GetByKey,
